    ...
    var wiz_ajax = {
        ...
        init: function () {
			if (typeof console !== 'undefined' && console.error) {
				/*
				funzione puramente di analisi: class-wiz-ajax.php
				passa un array che può essere filtrato da plugins ed altro.
				controllare la console per verificare le variabili caricate.
				*/
				for (var prop in setup) {
				  console.log("[wiz-ajax.js] setup." + prop + " = " + setup[prop]);
				}
            }
            try {
		   ...
		   var scovolini_sum = ultra_fine + fine + medium + large + extra_large;
       	 if( scovolini_sum > 5 ){
					...
					$('#modal-generico').modal({
  						backdrop: 'static'
						}).find('.modal-body' ).html('<p>' + setup.text_box_4 + '</p>');
	   ...
	   generateSum:function(){
		   ...
		   if ( ! isNaN ( scovolini_sum ) ) {
				...
				if( scovolini_sum === 0 ){
					...
					$('#modal-generico').modal({
  						backdrop: 'static'
						}).find('.modal-body' ).html('<p>' +setup.text_box_1 + '</p>');
				}
					...
					$('#modal-generico').modal({
  						backdrop: 'static'
						}).find('.modal-body' ).html('<p>' + setup.text_box_2 + '</p>');
				 ...
				 if( scovolini_sum === 5 ){
					...
					$('#modal-generico').modal({
  						backdrop: 'static'
						}).find('.modal-body' ).html('<p>' +setup.text_box_3 + '</p>');
        ...
        afterEditorComplete: function (event) {
			...
			/* evitiamo che il configuratore venga caricato se il prodotot è il box non configurabile */
			
			if( Number(cookie.wizard_product_id) !== setup.box_nocustom /* 2567 in italiano*/ ){
		...
		setImage_spazzolino: function(){
			...
			return $.ajax({
				...
				success: function(){
					console.log('creazione immagine spazzolino' );
					wiz_ajax.setPost();
		...
		setImage_kit: function(){
			...
			return $.ajax({
				...
				success: function(){
					console.log('creazione immagine kit' );
					wiz_ajax.setPost();
				}			
			});
		},
